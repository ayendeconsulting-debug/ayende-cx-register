...
} from "../services/crmIntegrationService.js";
...
router.post("/customer-update", async (req, res) => {
  try {
    ...
    if (!customerId && !customerData.email) {
      return res
        .status(400)
        .json(errorResponse("Customer ID or email required"));
    }
    ...
    res
      .status(500)
      .json(errorResponse("Failed to update customer", error.message));
  }
});
...
router.post("/marketing-prefs", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json(errorResponse("Failed to update preferences", error.message));
  }
});
...
router.get("/customer/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json(errorResponse("Failed to fetch customer", error.message));
  }
});
...
router.get("/transactions", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json(errorResponse("Failed to fetch transactions", error.message));
  }
});
...
router.get("/products", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json(errorResponse("Failed to fetch products", error.message));
  }
});
...
router.get("/sync-status", async (req, res) => {
  try {
    ...
    const [successCount, failedCount, pendingCount, lastSync] =
      await Promise.all([
    ...
    const successRate =
      totalSyncs > 0 ? ((successCount / totalSyncs) * 100).toFixed(2) : 0;
    ...
    res
      .status(500)
      .json(errorResponse("Failed to fetch sync status", error.message));
  }
});
